...
int alto, ancho;
int indice=0;
vector < pair<float,float> > lines, puntos_recorridos;
...
vector <pair<vector<int>,float>> mejores_caminos_desordenado;
...
///////////////////////////////////////////////OTROS PUNTOS//////////////////////////////////////////////////////
    ...
    vector<int> mejor= mejores_caminos[0].first;
    for(int j=0;j<mejor.size();j++)
	puntos_recorridos.push_back(puntos[ mejor[j] ]);
    //puntos_recorridos=mejores_caminos[0].first;
    for(int i =0; i<puntos_recorridos.size()-1; i++)
    {
        glVertex3f(puntos_recorridos[i].first,puntos_recorridos[i].second,0.0f);
	glVertex3f(puntos_recorridos[i+1].first,puntos_recorridos[i+1].second,0.0f);
    }
    glEnd();
    //////////////////////////////////////////INICIO//////////////////////////////ROJO
    glPointSize(10.5);
    ...
    glVertex3f(puntos_recorridos[0].first,puntos_recorridos[0].second,0.0f);
    glEnd();
    ///////////GRAFICOS
    glColor3f(1.0f,1.0f,1.0f); 
    glRectf(maxx+1,miny,maxx+100, maxy);
  ...
  ///////////////////////////////////////////////PROMEDIOS//////////////////////////////////////////////////////
    ...
    {
        glVertex3f(maxx+2+i*0.5,miny,0.0f);
	glVertex3f(maxx+2+i*0.5,(promedios[i]-mejores_caminos[0].second+0.1),0.0f);
	//cout<<(mejores_caminos_desordenado[i].second)*0.1<<endl;
    }
    //cout<<"GG"<<endl;
    glEnd();
    ///////////////////////////////////////////////BARRAS//////////////////////////////////////////////////////
   ...
   // glLineWidth(3.0);
    ...
    //puntos_recorridos=mejores_caminos[0].first;
    for(int i =0; i<mejores_caminos_desordenado.size()-1; i++)
    {
        glVertex3f(maxx+2+i*0.5,miny,0.0f);
	glVertex3f(maxx+2+i*0.5,(mejores_caminos_desordenado[i].second-mejores_caminos[0].second+0.1),0.0f);
	//cout<<(mejores_caminos_desordenado[i].second)*0.1<<endl;
    }
    //cout<<"GG"<<endl;
    glEnd();
...
int main(int argc, char **argv)
{
    ...
    cout<<"Los caminos Generados son: "<<endl;
    for(int i=0;i<todos_caminos.size();i++)
    {
        cout<<"La distancia de este Camino es: "<<todos_caminos[i].second<<" ";
        for(int j=0;j<todos_caminos[i].first.size();j++)
        {
            cout<<todos_caminos[i].first[j]<<" ; ";
        }
        cout<<endl;
    }

    /*Empezamos con la Generación de la Nueva Población*/
    /*sort( todos_caminos.begin(), todos_caminos.end() );
    cout<<"Again "<<endl;
    for(int i=0;i<todos_caminos.size();i++)
    {
        cout<<"La distancia de este Camino es: "<<todos_caminos[i].second<<" ";
    ...
    int nro_it=100;
    seleccion();
    for(int i=0;i<nro_it;i++)
    {
        seleccionAleatoria(todos_caminos,individuos);
        escogerPoblacion(todos_caminos,individuos);

    }
    mejores_caminos_desordenado=mejores_caminos;
     sort(mejores_caminos.begin(), mejores_caminos.end(), [](const std::pair<vector<int>,float>&left, const pair<vector<int>,float> &right) {
